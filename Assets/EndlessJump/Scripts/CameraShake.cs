using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class CameraShake : MonoBehaviour
{
    public float jumpIteration = 4.5f;

    public void shake()
    {
        float height = Mathf.PerlinNoise(jumpIteration, 0f) * 5;
        height = height * height * 0.2f;

        float shakeAmt = height; // degrees to shake the camera
        float shakePeriod = 0.25f;  //each shake time period
        float dropOffTime = 1.25f;  //shake settle down time

        LTDescr shakeTween = LeanTween.rotateAroundLocal(gameObject, Vector3.right, shakeAmt, shakePeriod).setEase(LeanTweenType.easeShake).setLoopClamp().setRepeat(-1);
        LeanTween.value(gameObject, shakeAmt, 0, dropOffTime).setOnUpdate((float val) => { shakeTween.setTo(Vector3.right * val); }).setEase(LeanTweenType.easeOutQuad);
    }

}
